...
def split_data_into_passports(data: ty.List) -> ty.List[ty.Dict]:
...
def check_year(input_year, min_year, max_year):
    """ Utility function to check year range. Inclusive """
    return min_year <= int(input_year) <= max_year


def check_height(input_height):
...
def check_hair_color(input_hair_color):
...
def check_eye_color(input_eye_color, allowed_eye_colors):
...
def check_passport_id(input_pid):
